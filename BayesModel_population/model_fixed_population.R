model{
  # Prior ----
    ...
    xi ~ dnorm(0, ninfo)
    for(j in 1:Nsp){ b[j, 1:4] ~ dmnorm(B[], TAU[,]) }
    for(m in 1:4){ B[m] ~ dnorm(0,ninfo) }
    
    TAU[1:4,1:4] ~ dwish(W[,], 5)
    SIGMA[1:4,1:4] <- inverse(TAU[,])
  ...
  # Likelihood ----
    ...
    for(i in 1:Nsite){
      for(j in 1:Nsp){
        ...
        log.lambda[i,j] <- x[HUC12[i],j] + b[j,4]*scl.ELEV[i] + R[ YEAR[i] ]
      }
    }
    ...
    for(k in 1:K){
      ...
      for(j in 1:Nsp){
        ...
        x_mu[k,j] <- b[j,1] + xi*scl.TSS[k] + b[j,2]*scl.TN[k] + b[j,3]*scl.TP[k]
      }
    }
    ...
    for(j in 1:Nsp){
      ## unscaled parameters
      Alpha[j] <- b[j,1] - xi*( mean(TSS[])/sd(TSS[]) )
      Beta[j] <- xi/sd(TSS[])
